  ...
  <q-form class="row" @submit.prevent="updatePassword">
    <q-card square flat bordered class="col q-pa-sm">
      <q-card-section>
        ...
        <p>Pastikan akun anda menggunakan kata sandi acak yang panjang untuk tetap aman.</p>
        <q-input square v-model="form.current_password" label="Kata Sandi Sekarang" type="password" lazy-rules
        ...
        <q-input square v-model="form.password" label="Kata Sandi Baru" type="password" lazy-rules
          :disable="form.processing" :error="!!form.errors.password" :error-message="form.errors.password"
          :rules="[(val) => (val && val.length > 0) || 'Kata sandi harus diisi.']" />
        <q-input square v-model="form.password_confirmation" label="Konfirmasi Kata Sandi" type="password" lazy-rules
